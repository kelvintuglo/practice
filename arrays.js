const num = [1, 2, 3];
num.push(4);
console.log(num);

const poppednum = num.pop();
console.log(poppednum);
console.log(num);

const _names = ["esi", "kojo", "ama"];
_names.push("yaw");
console.log(_names);
console.log(_names.length);
_names.pop();
console.log(_names);
console.log(_names.length);

const shoes = ["vans", "converse", "air force", "jordans"];
shoes.unshift("birkenstocks", "new balance");
console.log(shoes);

shoes.shift("birkenstocks");
console.log(shoes);

//Array Destructuring
const cars = ["bmw", "lamborghini", "ferrari", "toyota"];
const [firstCar, secondCar, thirdCar] = cars;
console.log(firstCar);
console.log(secondCar);
console.log(thirdCar);

const [myCar, , herCar] = cars;
console.log(myCar);
console.log(herCar);

const [theCar, thatCar, ,] = cars;
console.log(theCar);
console.log(thatCar);

const [a, b, c, d, e = "ford"] = cars;
console.log(a);
console.log(b);
console.log(c);
console.log(d);
console.log(e);

const [whip, guap, ...rest] = cars;
console.log(whip);
console.log(guap);
console.log(rest);

const str = "hello";
const splitStr = str.split("");
console.log(splitStr);


const msg = "hello, how are you doing";
const splitMsg = msg.split(" ");
console.log(splitMsg);

const letter = "one-dimensional";
const splitLetter = letter.split("-");
console.log(splitLetter);

const arr = "two-dimensional arrays are so fun dude";
const splitArr = arr.split("-");
console.log(splitArr);

let Arr = [1, 2, 3, 4, 5];
let removed = Arr.splice(1, 2);
console.log(Arr, removed);

let _arr = ["a", "b", "c"];
console.log(_arr.indexOf("d"));
console.log(_arr.includes("d"));

let nums = [1, 2, 3];
let doubled = nums.map(n => n * 2);
console.log(nums, doubled);

//Arrays mini project
let students = ["Ama", "Kwame", "Yaw", "Efua"];
let grades = [78, 65, 90, 55];

//Roster Management
function addStudent (name, grade) {
  students.push();
  grades.push();

  return `Student name: ${students.push()}.
  Grade: ${grades.push()}.`;
  console.log(students.push());
  console.log(grades.push());
}
console.log(addStudent("Kojo", 89));

function addPriority (name, grade) {
  students.unshift();
  grades.unshift();

  return `Student's name: ${students.unshift()}.
  Grade: ${grades.unshift()}`;
}
console.log(addPriority("Adjoa", 30));

function removeLastStudent () {
  students.pop();
  grades.pop();

  return `Removed name: ${students.pop()}
  Removed grade: ${grades.pop()}`;
}
console.log(removeLastStudent());

function removeFirstStudent () {
  students.shift();
  grades.shift();

  return `Removed student: ${students.shift()}
  Removed grade: ${grades.shift()}`;
}
console.log(removeFirstStudent());

//2D arrays
const _2D = [[1, 2, 3], [4, 5, 6], [7, 8, 9]];
console.log(_2D[0]);
console.log(_2D[1]);
console.log(_2D[2]);

console.log(_2D[0][0]);
console.log(_2D[1][2]);
console.log(_2D[2][2]);

//push
_2D[0].push(14);
_2D[1].push(15);
_2D[2].push(16);
console.log(_2D);

//pop
_2D[0].pop();
_2D[1].pop();
_2D[2].pop();
console.log(_2D);

//unshift
_2D[0].unshift(0);
_2D[1].unshift(11);
_2D[2].unshift(22);
console.log(_2D);

//shift
_2D[0].shift();
_2D[1].shift();
_2D[2].shift();
console.log(_2D);

//Destructuring
const iceCream = ["vanilla", "chocolate", "strawberry"];
const [firstFlavor, secondFlavor, thirdFlavor] = iceCream;
console.log(firstFlavor);
console.log(secondFlavor);
console.log(thirdFlavor);

console.log(iceCream);

const socMedia = ["Instagram", "X", "Pinterest", "YouTube"];
const [one, two, three, four] = socMedia;
console.log(one);
console.log(two);
console.log(three);
console.log(four);

//Splice
const _nums = [1, 2, 3, 4, 5, 6];
_nums.splice(1, 2);
_nums.splice(1, 0, 2, 3);
console.log(_nums);

const colors = ["yellow", "orange", "green", "blue"];
colors.splice(0, 0, "purple", "pink");
colors.splice(2, 2, "indigo", "red", "black");
console.log(colors);
console.log(colors.includes("black"));
console.log(colors.includes("orange"));
console.log(colors.length);
console.log(colors.indexOf("pink"));
console.log(colors.includes("magenta"));

//Split
const statement = "Wagwan";
const splitStatement = statement.split("");
const splitStatement2 = statement.split(" ");
const splitStatement3 = statement.split("-");

console.log(statement);
console.log(splitStatement);
console.log(splitStatement2);
console.log(splitStatement3);
console.log(splitStatement.reverse());

const $msg = "Pour from a full cup";
const $msg1 = $msg.split("");
const $msg2 = $msg.split(" ");
const $msg3 = $msg.split("-");

console.log($msg);
console.log($msg1);
console.log($msg2);
console.log($msg3);

const _msg = "by-the-way";
const _msg1 = _msg.split("");
const _msg2 = _msg.split(" ");
const _msg3 = _msg.split("-");

console.log(_msg);
console.log(_msg1);
console.log(_msg2);
console.log(_msg3);

console.log(_msg1.join(""));
console.log(_msg1.join(" "));
console.log(_msg1.join("-"));